...
const Attendent = () => {
    ...
    const getDatasecondFilteredStudents = () => {
      return getFilteredStudents().filter((student) => student.status === true);
    };

    const getDatasecondFilteredStudentsStatus = () => {
      return getFilteredStudents().filter((student) => (student.status === true && student.status === false) );
    };
    
    const filteredStudents = getDatasecondFilteredStudents().filter((student) => {
    ...
    const visibleAll = getDatasecondFilteredStudents();

    //const visibleStatus = getDatasecondFilteredStudentsStatus();

    const data = [
  { id: 0, value: `${visibleStudents.length}`, label: ` ${monthFilter || 'All Time'}` },
  { id: 1, value: `${visibleAll.length}`, label: 'All Attendance' },
  { id: 2, value: `${attendent.length}`, label: 'Class Days' },
    ];
  ...
  return (
     <div>
        ...
        <div className="data-table-info">
          ...
          <div className="filter-options">
            ...
            <input
              ...
              style={{ fontSize: '16px', marginLeft: '10px', marginBottom: '10px', borderRadius: '4px', border: '1px solid #ccc', backgroundColor: '#fff', color: '#333' }}
            />
          </div>
        </div>
      ...
      {/* <p>Number of Attendance : {visibleStudents.length}</p> */}
